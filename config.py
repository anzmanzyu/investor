...
# ─── 損切り・利確 ────────────────────────────────────────
STOP_LOSS_PERCENT   = 4.0        # エントリーからの損切り幅（%）
TAKE_PROFIT_PERCENT = 8.0        # エントリーからの利確幅（%）
...
MIN_VOLUME_PER_DAY  = 100_000    # 1日最低出来高（株）。これ以下は流動性不足として除外
VOLUME_RATIO_MIN    = 1.2        # 当日出来高 / 20日平均。この値以上のみ通過
